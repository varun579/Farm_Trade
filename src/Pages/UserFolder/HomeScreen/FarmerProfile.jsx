import { StyleSheet, Text, View,Image, TouchableOpacity,ScrollView } from 'react-native'
import React from 'react'
import { useNavigation, useRoute } from '@react-navigation/native'



export default function FarmerProfile() {

  const route=useRoute();
  const navigation =useNavigation()
  const name=route.params.name
  const image=route.params.image
  const farmerId=route.params.farmerId
 


  return (
    <View style={{backgroundColor:'white'}}>
      <Image source={{uri:'https://imgs.search.brave.com/Q53tGbDjk8q40Kqh_yhTpQ0RomA7M91MGSdlw88YOs8/rs:fit:500:0:0:0/g:ce/aHR0cHM6Ly9jZG4u/cGl4YWJheS5jb20v/cGhvdG8vMjAxNi8w/Ni8xOC8yMS81Ni9j/YXVsaWZsb3dlci0x/NDY1NzMyXzY0MC5q/cGc'}}
         
         style={{height:250}}
       />
       <View style={styles.about}>
       
         <View style={styles.pic}>
            { image ? <Image
              
              source={{uri:`http://192.168.1.244:5000/${image}`}}
              style={{
                height:160,
                width:160,
                borderRadius:80
              }}
            /> :
            
            <Image

              source={{uri:`data:image/jpeg;base64,`}}
              style={{
                height:160,
                width:160,
                borderRadius:80
              }}
    
                  />
             }
         </View>
       </View>
       
       <ScrollView>
        
       <View style={styles.details}>
        
            <Text style={{color:'black', fontSize:23,fontWeight:'500'}}>{name}</Text>
            <Text  style={{fontSize:20,color:'black',fontWeight:'500'}}>About his Farm</Text>
      </View>
     
     
        <View style={styles.description}>
        <Text  style={{fontSize:20,color:'black',lineHeight:28,}}>Welcome to  my farm! We  grow  fresh, healthy produce with care. From fruits and vegetables to other farm products, everything is cultivated with pride. Feel free to ask—we're excited to share our harvest with you.</Text>
        <Text  style={{fontSize:20,color:'black',lineHeight:28}}>Thank you for supporting local farming</Text>
        </View>

        <TouchableOpacity style={styles.button} onPress={()=>navigation.navigate('FarmerSec',{farmerId:farmerId})}>
          <Text style={{fontSize:20,color:'white',fontWeight:'500'}}>Buy From Farmer</Text>
        </TouchableOpacity>

        </ScrollView>
     



    </View>
  )
}

const styles = StyleSheet.create({
    about:
    {
       height:100,
       backgroundColor:'white',
       borderTopRightRadius:30,
       borderTopLeftRadius:30,
       marginTop:-50,
       alignItems:'center'
       
    },
    pic:
    {
        borderWidth:12,
        borderColor:'white',
        height:180,
        width:180,
        borderRadius:90,
        position:'absolute',
        alignItems:'center',
        top:-80
        
        
    


    },
    details:
    {
       marginTop:20,
       alignItems:'center'
    },
    description:
    {
      
      
      marginTop: 0,
      padding: 20, 
      letterSpacing: 1,
 

      
    
      
      
    },
    button:
    {
      backgroundColor:'green',
      marginTop:20,
      marginHorizontal:20,
      height:50,
      alignItems:'center',
      justifyContent:'center',
      borderRadius:10,
      marginBottom:60
   

    }
})